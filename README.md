...
Во второй версии web-интерфейс есть только как локальный UI поверх ядра AI-Boss. В нём пока нет auth, пользовательских ролей, публичного server mode, фонового daemon-режима и очереди задач. Также нет Telegram-бота, SQLite, Redis, Docker, GitHub API и сложного RAG. Цель v2 — крепкое локальное ядро с прозрачной историей задач, интерактивным shell, local-only web UI, безопасным Codex-only write path и ограниченным fix-loop после ревью.
