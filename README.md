...
- web-очередь сохраняет запуски в `99_System/web-jobs.json`, чтобы длинные операции не блокировали интерфейс и были видны после перезапуска.
...
Команда `ai-boss status` проверяет их наличие локально и не отправляет реальные запросы.
Если CLI установлен через `nvm` или подключается только в интерактивном shell, AI-Boss дополнительно пробует найти реальный путь через `bash -ic "command -v <tool>"`.
...
- отправить длинную операцию в persistent web-очередь, отменить или повторить запуск и следить за live log;
- менять безопасные настройки `config.yaml` из local-only Settings;
...
Во второй версии web-интерфейс есть только как локальный UI поверх ядра AI-Boss. В нём пока нет auth, пользовательских ролей, публичного server mode и фонового daemon-режима. Также нет Telegram-бота, SQLite, Redis, Docker, GitHub API и сложного RAG. Цель v2 — крепкое локальное ядро с прозрачной историей задач, интерактивным shell, local-only web UI, безопасным Codex-only write path, persistent web-очередью и ограниченным fix-loop после ревью.
